...
| Compose-backed API keeps one side effect across restart and replay | `bash ./scripts/container-proof.sh`; public CI run [`31891411678`](https://github.com/dbett4/hermes-enterprise-deployment-lab/actions/runs/31891411678) at `3da5938` | The green Docker-backed job shows negative auth leaves count=0; post-commit 500 leaves count=1; after `enterprise-api` restart count is still 1; same key replays; demo against that API still ends with one side effect. The uploaded receipt is bound to the exact commit | Proves fixture JSON on a volume, not production storage, K8s, or OIDC |
...
| Stage-1 LangGraph cited retrieval, safety review, and evaluation | `.venv/bin/python -m pytest tests/test_agent_workflow.py -q`, `.venv/bin/python scripts/agent-workflow-proof.py`, and public CI run [`31891411678`](https://github.com/dbett4/hermes-enterprise-deployment-lab/actions/runs/31891411678) at `3da5938` | The CI-attested receipt has `evaluation_passed: true`, empty `executed_actions`, and hashed document/action provenance checked against the proof boundary's separately supplied authoritative fixture, plus the pinned `input_sha256` / `result_sha256` | The evaluator rejects missing external authority, self-signed provenance, citation laundering, and changed authoritative action fields, but remains a regression check rather than authz. The fixture is not signed. No vector store, model call, action executor, authz, or external validation. |

## Stage-1 LangGraph proof details

The hardened Stage-1 synthetic proof is public CI-attested at `3da5938` / run
[`31891411678`](https://github.com/dbett4/hermes-enterprise-deployment-lab/actions/runs/31891411678).
It remains separate from the deployment/recovery/telemetry/container evidence.
