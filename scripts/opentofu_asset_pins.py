"""Repository-pinned OpenTofu release asset digests for cloud IaC proof.

Linux/amd64 and Linux/arm64 digests for OpenTofu 1.12.5 are pinned in-repo.
`scripts/cloud-iac-proof.sh` requires actual == repository_pin == upstream
SHA256SUMS before extraction. This is not TOFU against the download channel
alone.
"""
...
PINNED_SHA256_BY_ARCH: dict[str, str] = {
    "amd64": "a6894d45ae7a17ce83189cce8fe04b5a65f68cefceb62455b5a6a89fa53ab38f",
    "arm64": "e67e9da2b1ddf5050ebee62a584cb826eafe1dfd3827d7ec20899ac62791ed1a",
}
...
def asset_arch_from_uname(machine: str) -> str:
    ...
    if normalized in {"aarch64", "arm64"}:
        return "arm64"
    raise ValueError(f"unsupported OpenTofu proof architecture: {machine}")
...
def repository_pin_for_arch(arch: str) -> str:
    try:
        ...
        raise ValueError(f"unsupported OpenTofu proof architecture: {arch}") from exc
...
def assert_three_way_equality(
    *,
    ...
    if not (actual == repository_pin == upstream_manifest):
        raise SystemExit(
            "CLOUD_IAC_PROOF_FAIL: OpenTofu asset digest mismatch "
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            "usage: opentofu_asset_pins.py arch <uname-m> | "
            "pin <arch> | "
